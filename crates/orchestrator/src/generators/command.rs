...
const FLAGS_ADDED_BY_US: [&str; 3] = [ "--unsafe-rpc-external", "--no-telemetry", "--no-mdns"];
const OPS_ADDED_BY_US: [&str; 4] = ["--chain", "--name", "--rpc-cors", "--rpc-methods"];

pub fn generate_for_node(node: &NodeSpec, options: GenCmdOptions, _para_id: Option<u32> ) -> (String, Vec<String>) {
    ...
    let chain_spec_path = format!("{}/{}.json", options.cfg_path, "rococo-local"); // TODO: get chain from spec
    ...
    //let mut resolved_cmd = vec![final_args.join(" ")];
