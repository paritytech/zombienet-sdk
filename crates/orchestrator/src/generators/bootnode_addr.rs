...
pub fn generate(peer_id: &str, ip: &IpAddr, port: u16,  args: &Vec<String>, p2p_cert: &Option<String>) -> Result<String,GeneratorError> {
    let addr = if let Some(index) = args.iter().position(|arg| arg.eq("--lister-addr")) {
        let listen_value = args.get(index + 1).ok_or(GeneratorError::BootnodeAddrGeneration("can not generate bootnode address from args".into()))?;
