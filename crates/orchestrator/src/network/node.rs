...
pub struct NetworkNode {
    ...
    // since we can use it later for recalculate the (cmd, args) from a
    // modified spec.
    pub(crate) cmd_generator_opts: GenCmdOptions,
    // Store the context used to generate this node.
...
impl NetworkNode {
    ...
    /// autogenerated by zombienet.
    pub fn user_args(&self) -> Vec<String> {
        self.spec
            .args
            .iter()
            .fold(vec![], |acc, arg| [acc, arg.to_vec()].concat())
    }
    ...
    ///
    /// The node will be restarted with the same `env` and isolated dirrectory (e.g same database).
    ///
    /// Note: If you're using this method with the native provider on the attached network, the live network has to be running
    /// with global setting `teardown_on_failure` disabled.
    pub async fn restart_with(
        ...
        let (program, args) = match self.context {
            NodeContext::Rc
            | NodeContext::Para {
                is_cumulus_based: false,
                ..
            } => generate_node_command(&spec_cloned, self.cmd_generator_opts.clone(), None),
            NodeContext::Para {
                para_id,
                is_cumulus_based: true,
            } => generate_node_command_cumulus(
                &spec_cloned,
