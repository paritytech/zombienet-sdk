...
impl ChainSpec {
    ...
    pub async fn read_chain_id<'a, T>(&self, scoped_fs: &ScopedFilesystem<'a, T>) -> Result<String, GeneratorError>
    where T: FileSystem
    {
        let (content, _) = self.read_spec(scoped_fs).await?;
        let chain_spec_json: serde_json::Value = serde_json::from_str(&content).map_err(|_| GeneratorError::ChainSpecGeneration("Can not parse chain-spec as json".into()))?;
        if let Some(chain_id) = chain_spec_json.get("id") {
            if let Some(chain_id) = chain_id.as_str() {
                Ok(chain_id.to_string())
            } else {
                Err(GeneratorError::ChainSpecGeneration("id should be an string in the chain-spec, this is a bug".into()))
            }
        } else {
            Err(GeneratorError::ChainSpecGeneration("'id' should be a fields in the chain-spec of the relaychain".into()))
        }
    }
    ...
    async fn write_spec<'a, T>(&self, scoped_fs: &ScopedFilesystem<'a, T>, content: impl Into<String>) -> Result<(), GeneratorError>
    where T: FileSystem
    {
        ...
        scoped_fs.write(path, content.into()).await.map_err(|_| GeneratorError::ChainSpecGeneration(format!("Can not write chain-spec from {}", path.to_string_lossy() )))?;

        Ok(())
    }

    // TODO: (javier) move this fns to state aware
    pub async fn customize_para<'a, T>(&self, para: &ParachainSpec, relay_chain_id: &str, scoped_fs: &ScopedFilesystem<'a, T>) -> Result<(), GeneratorError>
    where T: FileSystem
    {
        ...
        if let ChainSpecFormat::Plain = format {
            let pointer = get_runtime_config_pointer(&chain_spec_json).map_err(|e| GeneratorError::ChainSpecGeneration(e))?;
            ...
            let validators: Vec<&NodeSpec> = para.collators.iter().filter(|node| node.is_validator).collect();
            ...
            let _key_type = if let Some(_) = chain_spec_json.pointer(&format!("{}/session",pointer)) {
                ...
                let invulnerables: Vec<&NodeSpec> = para.collators.iter().filter(|node| node.is_invulnerable).collect();
            ...
            // write spec
            let content = serde_json::to_string_pretty(&chain_spec_json).map_err(|_| GeneratorError::ChainSpecGeneration("can not parse chain-spec value as json".into()))?;
    ...
    pub async fn customize_relay<'a, T, U>(&self, relaychain: &RelaychainSpec, _hrmp_channels: &Vec<HrmpChannelConfig>, para_artifacts: Vec<ParaGenesisConfig<U>>, scoped_fs: &ScopedFilesystem<'a, T>) -> Result<(), GeneratorError>
    where T: FileSystem, U: AsRef<Path>
    {
        let (content, format) = self.read_spec(scoped_fs).await?;
        let mut chain_spec_json: serde_json::Value = serde_json::from_str(&content).map_err(|_| GeneratorError::ChainSpecGeneration("Can not parse chain-spec as json".into()))?;
        ...
        if let ChainSpecFormat::Plain = format {
            ...
            for para_genesis_config in para_artifacts.iter() {
                add_parachain_to_genesis(&pointer, &mut chain_spec_json, para_genesis_config, &scoped_fs).await.map_err(|e| GeneratorError::ChainSpecGeneration(e.to_string()))?;
            }
    ...
    pub async fn add_bootnodes<'a, T>(&self, scoped_fs: &ScopedFilesystem<'a, T>, bootnodes: &Vec<String>) -> Result<(), GeneratorError>
    where T: FileSystem
    {
        let (content, _) = self.read_spec(scoped_fs).await?;
        let mut chain_spec_json: serde_json::Value = serde_json::from_str(&content).map_err(|_| GeneratorError::ChainSpecGeneration("Can not parse chain-spec as json".into()))?;
        ...
        if let Some(bootnodes_on_file) = chain_spec_json.get_mut("bootNodes") {
            if let Some(bootnodes_on_file) = bootnodes_on_file.as_array_mut() {
                let mut bootnodes_to_add = bootnodes.iter().map(|bootnode| json!(bootnode)).collect();
                bootnodes_on_file.append(&mut bootnodes_to_add);
            } else {
                return Err(GeneratorError::ChainSpecGeneration("id should be an string in the chain-spec, this is a bug".into()));
            };
        } else {
            return Err(GeneratorError::ChainSpecGeneration("'bootNodes' should be a fields in the chain-spec of the relaychain".into()));
        };
...
async fn add_parachain_to_genesis<'a, T, U>(runtime_config_ptr: &str, chain_spec_json: &mut serde_json::Value, para_genesis_config: &ParaGenesisConfig<U>, scoped_fs: &ScopedFilesystem<'a, T>) -> Result<(), anyhow::Error>
where T: FileSystem, U: AsRef<Path>
{
    println!("es:");
    ...
    if let Some(val) = chain_spec_json.pointer_mut(runtime_config_ptr) {
        ...
        let paras = val.pointer_mut(&paras_pointer).ok_or(anyhow!("paras pointer should be valid {:?} ", paras_pointer))?;
...
fn add_aura_authorities(runtime_config_ptr: &str, chain_spec_json: &mut serde_json::Value, nodes: &Vec<&NodeSpec>, _key_type: KeyType) {
    if let Some(val) = chain_spec_json.pointer_mut(runtime_config_ptr) {
        let keys: Vec<String> = nodes.iter().map(|node| {
            node.accounts.accounts.get("sr")
    ...
    // // current version (ts).
    // fn para_custom() { todo!() }
fn override_parachain_info(runtime_config_ptr: &str, chain_spec_json: &mut serde_json::Value, para_id: u32) {
    if let Some(val) = chain_spec_json.pointer_mut(runtime_config_ptr) {
...
fn add_collator_selection(runtime_config_ptr: &str, chain_spec_json: &mut serde_json::Value, nodes: &Vec<&NodeSpec>) {
    if let Some(val) = chain_spec_json.pointer_mut(runtime_config_ptr) {
        let keys: Vec<String> = nodes.iter().map(|node| {
            node.accounts.accounts.get("sr")
            .expect("'sr' account should be set at spec computation, this is a bug")
