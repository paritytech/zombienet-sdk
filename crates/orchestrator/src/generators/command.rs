...
const FLAGS_ADDED_BY_US: [&str; 5] = [ "--unsafe-rpc-external", "--no-telemetry", "--no-mdns", "--collator", "--" ];
const OPS_ADDED_BY_US: [&str; 5] = ["--chain", "--name", "--rpc-cors", "--rpc-methods", "--parachain-id" ];

// TODO: can we abstract this and use only one fn (or at least split and reuse in small fns)
pub fn generate_for_cumulus_node(node: &NodeSpec, options: GenCmdOptions, para_id: u32) -> (String, Vec<String>) {
    let NodeSpec {key, args, is_validator, bootnodes_addresses, ..} = node;

    let mut tmp_args: Vec<String> = vec![
        "--node-key".into(), key.clone(),
    ];
    ...
    if !bootnodes_addresses.is_empty() {
        ...
        let bootnodes = bootnodes_addresses.iter().map(|m| {
            m.to_string()
    ...
    let node_specific_bootnodes: Vec<String> = node.bootnodes_addresses.iter().map(|b| b.to_string()).collect();
    ...
    let mut full_node_args_filtered = full_node_args.iter().filter_map(|arg| {
        match arg {
            Arg::Flag(flag) => {
                if FLAGS_ADDED_BY_US.contains(&flag.as_str()) { None }
                else { Some(flag.to_owned()) }
            },
            Arg::Option(k, v) => {
                if OPS_ADDED_BY_US.contains(&k.as_str()) { None }
                else {
    ...
    let mut args_filtered = collator_args.iter().filter_map(|arg| {
        match arg {
            Arg::Flag(flag) => {
                if FLAGS_ADDED_BY_US.contains(&flag.as_str()) { None }
                else { Some(flag.to_owned()) }
...
pub fn generate_for_node(node: &NodeSpec, options: GenCmdOptions, para_id: Option<u32> ) -> (String, Vec<String>) {
    let NodeSpec {key, args, is_validator, bootnodes_addresses, ..} = node;
    let mut tmp_args: Vec<String> = vec![
        "--node-key".into(), key.clone(),
    ...
    let listen_value = if let Some(listen_val) = args.iter().find_map(|arg|
        match arg {
            Arg::Flag(_) => None,
            Arg::Option(k, v) => {
                if  k.eq("--listen-addr")  { Some(v) }
                else { None }
            }
        }
    ) {
        ...
        let port_part = parts.get_mut(4).expect("should have at least 5 parts, this is a bug");
    ...
    let mut args_filtered = args.iter().filter_map(|arg| {
        match arg {
            Arg::Flag(flag) => {
                if FLAGS_ADDED_BY_US.contains(&flag.as_str()) { None }
                else { Some(flag.to_owned()) }
