...
pub fn generate_for_node(name: &str) -> Result<(String, String), GeneratorError> {
    ...
    let bytes = <[u8; 32]>::from_hex(key.clone()).map_err(|_| GeneratorError::IdentityGeneration("can not transform hex to [u8;32]".into()))?;
    let sk  = ed25519::SecretKey::try_from_bytes(bytes).map_err(|_| GeneratorError::IdentityGeneration("can not create sk from bytes".into()))?;
